...
export const katbot = {
  ...
  initAlreadyExists: () =>
    warn(`\n  ⚠️  vault already exists here. you want me to nuke it? (g-crypt nuke)\n`),
  ...
  noVault: () =>
    skull(`\n  ❌ no vault here. run 'g-crypt init' first, genius.\n`),
  genericError: (msg) =>
    skull(`\n  ❌ ${msg}\n`),
  ...
  pushNoVault: () =>
    skull(`\n  ❌ no vault to push. run 'g-crypt init' and store something first.\n`),
  pullSuccess: (files) =>
    neon(`\n  📥 pulled ${files.length} file${files.length !== 1 ? 's' : ''} from gist: ${files.join(', ')}\n`),
  pullNoGist: () =>
    warn(`\n  ⚠️  no gist linked. run 'g-crypt push' first or 'g-crypt link <gist-id>'.\n`),
  pullEmpty: () =>
    warn(`\n  ⚠️  gist exists but nothing to pull. weird.\n`),
  linkSuccess: (url) =>
    neon(`\n  🔗 linked to gist. run 'guttercrypt pull' to sync. ${url}\n`),
  linkNotVault: () =>
  ...
  recallEmpty: () =>
    warn(`\n  🧠 memory is empty. tell me something with 'g-crypt remember <note>'.\n`),
