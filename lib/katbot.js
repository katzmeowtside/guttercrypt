...
export const katbot = {
  ...
  pushNoVault: () =>
    skull(`\n  ❌ no vault to push. run 'guttercrypt init' and store something first.\n`),
  pullSuccess: (files) =>
    neon(`\n  📥 pulled ${files.length} file${files.length !== 1 ? 's' : ''} from gist: ${files.join(', ')}\n`),
  pullNoGist: () =>
    warn(`\n  ⚠️  no gist linked. run 'guttercrypt push' first or 'guttercrypt link <gist-id>'.\n`),
  pullEmpty: () =>
  ...
  recallEmpty: () =>
    warn(`\n  🧠 memory is empty. tell me something with 'guttercrypt remember <note>'.\n`),
